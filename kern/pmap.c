...
boot_alloc(uint32_t n)
{
	...
	nextfree = nextfree + ((n-1)/PGSIZE +1)*PGSIZE;
	return result;
}
...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		...
		if (i >= PGNUM(EXTPHYSMEM) && i < PGNUM(PADDR(nextfree))) {
			continue;
		}
		pages[i].pp_ref = 0;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	for(int i = 0; i < size ; i += PGSIZE){
		pte_t *pte_ptr = pgdir_walk(pgdir,(const void *)va + i,1);
		*pte_ptr = ( pa + i )| PTE_P | perm; 
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (; begin < end; begin += PGSIZE) {
		...
		if ((uintptr_t) begin >= ULIM || pte_p == NULL || ((*pte_p & (perm | PTE_P)) != (perm | PTE_P))) {
			user_mem_check_addr = (uintptr_t) MAX(begin, va);
			return -E_FAULT;
		}
	}
	return 0;
